...
const buttonAction = async(action,slug,intialQuantity) => {
  let index = products.findIndex((item)=> item.slug == slug)
  let newProducts = JSON.parse(JSON.stringify(products))
  if(action == "plus"){
    newProducts[index].quantity = parseInt(intialQuantity) + 1
  ...
  if(action == "plus"){
    newDropdown[indexdrop].quantity = parseInt(intialQuantity) + 1
  ...
  body: JSON.stringify({action,slug,intialQuantity})
});
...
<h1 className="text-3xl font-semibold mb-6">Search a Product</h1>
        ...
        { loading &&  <div className="flex justify-center items-center"><svg width="80px" height="80px" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  <circle cx="50" cy="50" r="45" stroke="#3498db" stroke-width="8" fill="none" stroke-linecap="round">
    <animate
      attributeName="stroke-dasharray"
      values="1, 150; 89, 150; 89, 150"
      keyTimes="0; 0.5; 1"
      dur="1.5s"
      repeatCount="indefinite"
    />
  </circle>
  <circle cx="50" cy="50" r="35" stroke="#e74c3c" stroke-width="8" fill="none" stroke-linecap="round" stroke-dasharray="1, 150">
    <animate
      attributeName="stroke-dasharray"
      values="1, 150; 89, 150; 89, 150"
      keyTimes="0; 0.5; 1"
      ...
      <div className="container  mx-auto my-8">
        ...
        <div className="mb-4">
          ...
          <div className="mb-4">
            ...
            <input name='quantity' value={productForm?.quantity || ""} onChange={handleChange} type="number" id="quantity" className="w-full border border-gray-300 px-4 py-2" />
          </div>

          <div className="mb-4">
            <label htmlFor="price" className="block mb-2">Price</label>
            <input name='price' value={productForm?.price || ""} onChange={handleChange} type="number" id="price" className="w-full border border-gray-300 px-4 py-2" />
          </div>
